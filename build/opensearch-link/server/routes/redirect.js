...
const state_builder_1 = require("../lib/state_builder");
...
function defineRoutes(router, core, logger) {
    router.get({
        ...
        const hasFilter = Object.keys(query).some((k) => !types_1.RESERVED_PARAMS.has(k));
        const hasQuery = query.query && query.query.length > 0;
        if (!hasFilter && !hasQuery) {
            return response.badRequest({
                body: {
                    message: 'At least one filter parameter or query is required. ' +
        ...
        try {
            ...
            if (result.total > 1000) {
                logger.warn(`opensearch-link: ${result.total} index patterns found but only first 1000 checked`);
            }
            const match = result.saved_objects.find((obj) => obj.attributes.title === indexName);
            if (match) {
                indexPatternId = match.id;
            }
            else {
                logger.warn(`opensearch-link: no index pattern found for "${indexName}", redirecting without index`);
            }
        }
        catch (err) {
        ...
        const discoverPath = (0, state_builder_1.buildDiscoverUrl)(query, indexPatternId);
